...
unsigned int* BeepBassAddr = NULL;
#define GPM4CON (*(volatile unsigned int*)LedBassAddr)     
#define GPM4DAT (*(volatile unsigned int*)(LedBassAddr+1)) 
#define GPD0CON (*(volatile unsigned int*)BeepBassAddr)
#define GPD0DAT (*(volatile unsigned int*)(BeepBassAddr+1))      
...
{
	int ret;
	LedBassAddr = (unsigned int*)ioremap(0x110002E0, 0x08);                                /*    LED地址灯映射     */
	BeepBassAddr = (unsigned int*)ioremap(0x114000A0, 0x08);
	GPM4CON &= ~((0x0f << 3 * 4) | (0x0f << 2 * 4) | (0x0f << 1 * 4) | (0xf << 0 * 4));    /*    IO口初始化   */
	...
	LED_Device = device_create(DeviceClass, NULL, dev, NULL, "DeviceDrive");
