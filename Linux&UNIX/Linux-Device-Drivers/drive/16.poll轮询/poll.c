...
unsigned int devicedrive_poll(struct file*fp, struct poll_table_struct* wait)
{
	unsigned int mask = 0;
	/*      调用poll_wait把当前进程添加到等待队列中      */
	poll_wait(fp, &key_wait, wait);         /*    这个函数不会引起进程的阻塞    */
	/*       判断按键是否被按下或者松开        */
	if (flage_press)
	{
		mask = POLLIN;                   /*   POLLIN可读     */
	}
	return mask;	/*     返回状态掩码(是否可读可写标志       */
}
/*       设备文件操作函数集结构体赋值      */
